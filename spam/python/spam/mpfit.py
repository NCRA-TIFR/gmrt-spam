...
# 20070130 HI: TODO - iterstop and iterkeystop
      ...
      if ( not parinfo is None ):
         ...
         if ( type( parinfo[ 0 ] ) != type( {} ) ):
            self.errmsg = 'ERROR: PARINFO must be a list of dictionaries.'
            return
         if ( ( not xall is None ) and ( len( xall ) != len( parinfo ) ) ):
         ...
         for i in range( npar ):
            ...
            if ( not limit[ 0 ] is None ):
               if ( xall[ i ] < limit[ 0 ] ):
                  ...
                  return
            if ( not limit[ 1 ] is None ):
               if ( xall[ i ] > limit[ 1 ] ):
                  self.errmsg = 'ERROR: parameters are not within PARINFO limits'
                  return
            if ( ( not limit[ 0 ] is None ) and ( not limit[ 1 ] is None ) and 
                   ( pfixed[ i ] == False ) ):
               if ( limit[ 0 ] > limit[ 1 ] ):
                  self.errmsg = 'ERROR: parameters are not within PARINFO limits'
                  return
...
# 20070130 HI: TODO - data type check
      ...
      while True:
         ...
         if ( nprint > 0 ) and ( not iterfunct is None ):
...
# 20070131 HI: TODO - Add external to check
      ...
      if ( not self.fnorm is None ) and ( not fnorm1 is None ):
         self.fnorm = ( max( [ self.fnorm, fnorm1 ] ) )**2

      self.covar = None
      self.perror = None
      ## (very carefully) set the covariance matrix COVAR
      if ( ( self.status > 0 ) and ( not nocovar ) and ( not n is None )
                     and ( not fjac is None ) and ( not ipvt is None ) ):
         sz = fjac.shape
         if ( n > 0 ) and ( len( sz ) > 1 ):
...
###############################################################################
   ...
   def parinfo( self, parinfo = None, key = 'a', default = None, n = 0 ):
      ...
      if ( ( n == 0 ) and ( not parinfo is None ) ):
         n = len( parinfo )
...
#            ans = mx * sqrt( ( ( vec / mx )**2 ).sum() )
         if ( mx > agiant ):
            ans = mx * sqrt( ( ( vec / mx )**2 ).sum() )
         elif ( mx < adwarf ):
            ans =  sqrt( ( ( vec * mx )**2 ).sum() ) / mx
         else:
            ans = sqrt( ( vec**2 ).sum() )
