...
import glob, os, signal, struct, errno, random
...
#            tid = Task.spawn(self, path, [name.upper() + ehex(popsno)], env)
            tid = Task.spawn(self, path, [name.upper() + ehex(popsno, 1, 0)], env)
...
#        start = '%-5s%d' % (self._params[tid].name.upper(), self._popsno[tid])
        start = '%-5s%s' % (self._params[tid].name.upper(), ehex(self._popsno[tid], 1, 0))
...
#                messages.append((priority, '%-5s%d: %s' % (task, popsno, msg)))
                messages.append((priority, '%-5s%s: %s' % (task, ehex(popsno, 1, 0), msg)))
                pass
            self._msgno[tid] += 1
            continue
...
def _allocate_popsno():
    # Remove any old lock files for this pid
    files = glob.glob('/tmp/AIPS?.' + str(os.getpid()))
    for file in files:
        try:
            os.unlink(file)
        except:
...
#    for popsno in range(1,36):
    popsno_range = []
    for i in range(16):
      popsno_range = popsno_range + range(1,1+16)
    random.shuffle( popsno_range )
    for popsno in popsno_range:
